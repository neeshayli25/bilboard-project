...
const getEffectiveRole = (role = '') => (role === 'user' ? 'advertiser' : role);
...
const compareAndUpgradePassword = async (user, plainPassword = '') => {
  if (!user?.password) return false;

  if (BCRYPT_HASH_PATTERN.test(String(user.password))) {
...
const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !(await compareAndUpgradePassword(user, password))) {
...
const getMe = async (req, res, next) => {
  try {
    ...
    res.json({
      ...userObject,
      role: getEffectiveRole(userObject.role),
      passkeyEnabled: Array.isArray(user.passkeys) && user.passkeys.length > 0,
