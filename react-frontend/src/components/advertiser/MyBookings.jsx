...
import { cancelBooking, displayMyBooking, getMyBookings, submitManualPayment } from "../../api";
...
export default function MyBookings() {
  ...
  const handleCancelBooking = async (booking) => {
    if (!window.confirm("Cancel this unpaid booking request? The slot will become available again.")) return;
    ...
    try {
      ...
      setFeedback({
        tone: "success",
        message: response.data?.message || "Booking cancelled before payment.",
        bookingId: booking._id,
      });
      setExpandedBookingId("");
      setFormState(emptyForm);
      ...
      setFeedback({
        tone: "error",
        message: error.response?.data?.message || "Could not cancel this booking.",
        bookingId: booking._id,
      });
    } finally {
      ...
      {bookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredBookings.map((booking) => {
            ...
            const canCancelBeforePayment = stage === "pending_review" || stage === "payment_required";
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {canCancelBeforePayment && (
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700 hover:bg-red-100 disabled:opacity-60"
                      >
                        {cancellingBookingId === booking._id ? (
                          <>
                            <LoaderCircle size={16} className="animate-spin" /> Cancelling
                          </>
                        ) : (
                          <>
                            <XCircle size={16} /> Cancel Before Payment
                          </>
                        )}
                      </button>
                    )}
