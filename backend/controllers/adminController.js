...
const toAdminBillboardPayload = (billboard) =>
  normalizeBillboardPricing({
    ...(billboard?.toObject ? billboard.toObject() : billboard),
    displayConfig: sanitizeDisplayConfigForAdmin(billboard.displayConfig),
  });

const applyBillboardPricing = (data = {}) => {
  const normalizedRate = getRatePerMinute(data);
  if (normalizedRate > 0) {
    data.pricePerHour = normalizedRate;
    data.pricePerMinute = normalizedRate;
  }
  return data;
};
...
export const getBillboards = async (req, res) => {
  ...
  res.json(billboards.map(toAdminBillboardPayload));
};
...
export const createBillboard = async (req, res) => {
  ...
  data.displayConfig = buildDisplayConfig(parseObjectField(data.displayConfig));

  const billboard = await Billboard.create(data);
  res.status(201).json(toAdminBillboardPayload(billboard));
};
...
export const updateBillboard = async (req, res) => {
  ...
  data.displayConfig = buildDisplayConfig({
    ...
    ...parseObjectField(data.displayConfig),
  });

  const updated = await Billboard.findByIdAndUpdate(req.params.id, data, { new: true });
  res.json(toAdminBillboardPayload(updated));
};
...
export const deleteBillboard = async (req, res) => {
  ...
  const protectedBookings = await Booking.find({
  ...
  if (protectedBookings.length) {
    return res.status(400).json({
      message:
        'This billboard has scheduled, completed, or payment-submitted bookings. Cancel or finish those records before deleting the billboard.',
      protectedBookings: protectedBookings.length,
    });
  }
  ...
  const openBookingIds = openBookings.map((booking) => booking._id);

  if (openBookingIds.length) {
    await Transaction.updateMany(
      {
        booking: { $in: openBookingIds },
    ...
    for (const booking of openBookings) {
      ...
      booking.adminReviewNote = 'Billboard was removed by admin before payment confirmation.';
      booking.gatewayMeta = {
        ...(booking.gatewayMeta || {}),
        manualPayment: {
          ...
          cancellationSource: 'billboard_delete',
    ...
    await Notification.insertMany(
      openBookings.map((booking) => ({
        ...
        message: `Your unpaid booking for ${billboard.name} was cancelled because the billboard was removed by admin before payment confirmation.`,
        type: 'booking',
  ...
  res.json({
    message: 'Billboard deleted completely.',
    cancelledBookings: openBookingIds.length,
  });
};

// ---------- Ad Approvals ----------
export const getAllAds = async (req, res) => {
  const ads = await Ad.find().populate('advertiser', 'name email phone organization').sort({ createdAt: -1 });
  res.json(ads);
};
...
export const getPendingAds = async (req, res) => {
  const ads = await Ad.find({ approvalStatus: 'pending' }).populate('advertiser', 'name email phone organization');
  res.json(ads);
};
...
export const getBookings = async (req, res) => {
  ...
  const bookings = await Booking.find({ billboard: { $in: billboardIds } })
    .populate('billboard', 'name city location pricePerHour pricePerMinute imageUrl size resolution type status displayConfig')
...
export const updateBookingStatus = async (req, res) => {
  ...
  if (status === 'cancelled' && ['submitted', 'paid'].includes(booking.paymentStatus)) {
    return res.status(400).json({
      message: 'Bookings with submitted or confirmed payment cannot be cancelled from this action.',
    });
  }
  ...
  if (status === 'cancelled') {
    ...
    booking.paymentStatus = 'pending';
    booking.checkoutExpiresAt = null;
    booking.adminReviewNote = 'Cancelled by admin before payment confirmation.';
    booking.gatewayMeta = {
      ...(booking.gatewayMeta || {}),
      manualPayment: {
        ...(booking.gatewayMeta?.manualPayment || {}),
  ...
  await Notification.create({
    ...
    message:
      status === 'cancelled'
        ? `Your unpaid booking ${booking._id} was cancelled by admin before payment confirmation.`
